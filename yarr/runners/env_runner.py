...
import numpy as np
import torch
from helpers.custom_rlbench_env import (CustomMultiTaskRLBenchEnv,
                                        CustomRLBenchEnv)
...
from yarr.utils.stat_accumulator import SimpleAccumulator, StatAccumulator
...
class EnvRunner(object):
    def __init__(
        ...
        self._train_replay_buffer = (
            train_replay_buffer
            if isinstance(train_replay_buffer, list)
            else [train_replay_buffer]
        )
        self._timesteps = (
            self._train_replay_buffer[0].timesteps
        ...
        if eval_replay_buffer is not None:
            eval_replay_buffer = (
                eval_replay_buffer
                if isinstance(eval_replay_buffer, list)
                else [eval_replay_buffer]
            )
        ...
        self._rollout_generator = (
            RolloutGenerator() if rollout_generator is None else rollout_generator
        )
        ...
        with self._internal_env_runner.write_lock:
            ...
            for name, transition, eval in self._internal_env_runner.stored_transitions:
                ...
                if add_to_buffer:
                    ...
                    rb = (
                        self._eval_replay_buffer[replay_index]
                        if eval
                        else self._train_replay_buffer[replay_index]
                    )
                    rb.add(
                ...
                self._current_task_id = (
                    transition.info["active_task_id"] if eval else -1
                )
