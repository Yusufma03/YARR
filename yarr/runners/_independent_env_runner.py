...
import os
import time
from multiprocessing import (Manager, Process, get_start_method,
                             set_start_method)
...
from yarr.agents.agent import Agent, ScalarSummary, Summary
...
class _IndependentEnvRunner(_EnvRunner):
    ...
    def _get_task_name(self):
        if hasattr(self._eval_env, "_task_class"):
            ...
            if self._eval_env.active_task_id != -1:
                task_id = (self._eval_env.active_task_id) % len(
                    self._eval_env._task_classes
                )
                eval_task_name = change_case(
                    self._eval_env._task_classes[task_id].__name__
                )
            else:
                eval_task_name = ""
            multi_task = True
    ...
    def _run_eval_independent(
        ...
        device = (
            torch.device("cuda:%d" % device_idx)
            if torch.cuda.device_count() > 1
            else torch.device("cuda:0")
        )
        with (
            writer_lock
        ):  # hack to prevent multiple CLIP downloads ... argh should use a separate lock
        ...
        if rec_cfg.enabled:
            ...
            cam_motion = CircleCameraMotion(
                cam, Dummy("cam_cinematic_base"), rec_cfg.rotate_speed
            )
        ...
        for n_eval in range(self._num_eval_runs):
            ...
            for ep in range(self._eval_episodes):
                ...
                logging.info(
                    "%s: Starting episode %d, seed %d." % (name, ep, eval_demo_seed)
                )
                ...
                print(
                    f"Evaluating {task_name} | Episode {ep} | Score: {reward} | Lang Goal: {lang_goal}"
                )
            ...
            if len(summaries) > 0:
                if multi_task:
                    task_score = [
                        s.value
                        for s in summaries
                        if f"eval_envs/return/{eval_task_name}" in s.name
                    ][0]
                else:
                    task_score = [
                        s.value for s in summaries if f"eval_envs/return" in s.name
                    ][0]
            else:
                task_score = "unknown"
