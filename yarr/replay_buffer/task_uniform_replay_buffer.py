...
class TaskUniformReplayBuffer(UniformReplayBuffer):
    ...
    def sample_index_batch(self, batch_size):
        ...
        while not found_indicies and attempt_count < 1000:
            ...
            sampled_tasks = list(
                np.random.choice(tasks, batch_size, replace=(batch_size > len(tasks)))
            )
            potential_indices = []
            for task in sampled_tasks:
                ...
                task_indices = self._task_idxs[task][
                    self._rank : total_size : self._num_replicas
                ]
    ...
    def get_transition_elements(self, batch_size=None):
        ...
        transition_elements = [
            ReplayElement(
                ACTION,
                (batch_size, self._timesteps) + self._action_shape,
                self._action_dtype,
            ),
            ReplayElement(
                REWARD,
                (batch_size, self._timesteps) + self._reward_shape,
                self._reward_dtype,
            ),
